...
from cloudware_server.common.const import BASE_SESSION_ID
...
def set_authorization(roles):
    """
    对于需要鉴权的接口，在 Route 类里面重写 roles() 方法
    """
    ...
    def wrapper(func):
        ...
        def inner_wrapper(**kwargs):
            ...
            session_id = cookies_dict.get(BASE_SESSION_ID)
            ...
            if not session_id:
                ...
                pf_session_info = CacheFactory.get_cache().get(session_id)
